...
from create_flat_weight import create_flat_weight
def train_model(events_sig, events_bkg,colsample_bytree_v ,gamma_v, learning_rate_v, max_depth_v, min_child_weight_v, n_estimators_v, reg_alpha_v, reg_lambda_v, subsample_v,samples, is_mass_window, weight_style, input_features ):
    ...
    events_sig['flat_mass_weight'] = 1
    # events_sig = create_flat_weight(events_sig, title = 'signal_')
    events_bkg_pp = events_bkg['pp'].arrays()
    events_bkg_pp['flat_mass_weight'] = 1
    events_bkg_pp = create_flat_weight(events_bkg_pp, title = 'pp_')
    events_bkg_dd = events_bkg['DataDriven_QCD'].arrays()
    events_bkg_dd['flat_mass_weight'] = 1
    # events_bkg_dd = create_flat_weight(events_bkg_dd, title = 'DataDriven_QCD_')
    ...
    df_sig = pd.DataFrame.from_records(events_sig[input_features].tolist())
    df_pfff = pd.DataFrame.from_records(events_bkg_dd[input_features].tolist())
    df_pp = pd.DataFrame.from_records(events_bkg_pp[input_features].tolist())
    df_sig_weight = pd.DataFrame.from_records(events_sig[['weight','sigmarvDecorr', 'flat_mass_weight']].tolist())
    df_bkg_pfff_weight = pd.DataFrame.from_records(events_bkg_dd[['weight','sigmarvDecorr', 'Norm_SFs','flat_mass_weight']].tolist())
    df_bkg_pp_weight = pd.DataFrame.from_records(events_bkg_pp[['weight','sigmarvDecorr', 'Norm_SFs','flat_mass_weight']].tolist())
    ...
    sig_total_weight = sig_total_weight * df_sig_weight['flat_mass_weight']
    bkg_pfff_total_weight = df_bkg_pfff_weight['weight'] * df_bkg_pfff_weight['Norm_SFs'] * df_bkg_pfff_weight['flat_mass_weight']
    bkg_pp_total_weight = df_bkg_pp_weight['weight'] * df_bkg_pp_weight['Norm_SFs'] * df_bkg_pp_weight['flat_mass_weight']
    

    
    # sig_total_weight = reweight_sig_bkg*sig_total_weight
    sig_total_weight = sig_total_weight # pay more attention on the signal
    # clip the negative weight to 0
    ...
    # save model
    pickle.dump(model, open("/hpcfs/cms/cmsgpu/zhangzhx/BDT/forcheck_" + zzTime+ samples + "_DiphotonXGboost_afterTune_new_withAllSigs.pkl", "wb"))
    # Plot ROC curve
    sig_dataframe = df_sig
    sig_dataframe['target'] = 1
    sig_dataframe['key'] = 'sig'
...
# events_sig_18 = uproot.open("/hpcfs/cms/cmsgpu/zhangzhx/eos/UL2018/MassUL2018_ETSS_PhoID/output_sig125.root:Sig125")
...
# train_model(events_sig=events_sig_18, events_bkg=events_bkg_18, colsample_bytree_v= 0.9810535004954177, gamma_v= 0.00505403851987829, learning_rate_v= 0.01019226479637405, max_depth_v= 4, min_child_weight_v= 6.372684449709314, n_estimators_v= 4463, reg_alpha_v= 2.8133658852186127, reg_lambda_v= 3.321265090765987, subsample_v= 0.8841130927443083, is_mass_window = False, samples="UL18_normal")
# train_model(events_sig=events_sig_18, events_bkg=events_bkg_18, colsample_bytree_v= 0.9810535004954177, gamma_v= 0.00505403851987829, learning_rate_v= 0.01019226479637405, max_depth_v= 4, min_child_weight_v= 6.372684449709314, n_estimators_v= 4463, reg_alpha_v= 2.8133658852186127, reg_lambda_v= 3.321265090765987, subsample_v= 0.8841130927443083, is_mass_window = True, samples="UL18_masswindow")
# ------------------------------------ --- ----------------------------------- #
...
# train_model(events_sig=events_sig_16preVFP, events_bkg=events_bkg_16preVFP, colsample_bytree_v = 0.9810535004954177, gamma_v = 0.00505403851987829, learning_rate_v = 0.03019226479637405, max_depth_v = 3, min_child_weight_v = 6.372684449709314, n_estimators_v = 4155, reg_alpha_v = 2.8133658852186127, reg_lambda_v = 3.321265090765987, subsample_v = 0.8841130927443083, samples="UL16PreVFP_no_masswindow_msweight_no_sigmarv_no_sigmawv", is_mass_window=False, weight_style = 'msweight', input_features=input_features)
