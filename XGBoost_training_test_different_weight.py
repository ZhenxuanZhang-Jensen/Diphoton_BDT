...
def train_model(events_sig, events_bkg,colsample_bytree_v ,gamma_v, learning_rate_v, max_depth_v, min_child_weight_v, n_estimators_v, reg_alpha_v, reg_lambda_v, subsample_v,samples ):
    ...
    # ---------------------------------- change sig weight to proper deal with negative weight in amc@nlo ---------------------------------- #
    # 取所有样本权重的绝对值，并计算它们的总和
    sample_weights = df_sig_weight['weight'].values
    # 计算处理前的权重直方图
    plt.hist(sample_weights, bins=100)
    plt.xlabel('Sample Weight')
    plt.ylabel('Frequency')
    plt.title('Histogram of Sample Weights Before Adjustment')
    plt.savefig('sample_weights_before.png')
    plt.cla()
    ...
    # 计算正权重样本和负权重样本的总和
    pos_weight = np.sum(abs_weights[sample_weights > 0])
    neg_weight = np.sum(abs_weights[sample_weights < 0])

    # 计算正权重样本和负权重样本的平均权重
    pos_avg_weight = total_weight / (2 * pos_weight)
    neg_avg_weight = total_weight / (2 * neg_weight)

    # 调整正权重样本的权重
    pos_indices = np.where(sample_weights > 0)[0]
    pos_weights = sample_weights[pos_indices] * (neg_avg_weight / pos_avg_weight)
    sample_weights[pos_indices] = pos_weights

    # 调整负权重样本的权重
    neg_indices = np.where(sample_weights < 0)[0]
    neg_weights = sample_weights[neg_indices] * (pos_avg_weight / neg_avg_weight)
    sample_weights[neg_indices] = neg_weights

    # 将所有样本的权重除以它们的总和，以使得它们的权重和为 1
    sample_weights = sample_weights / np.sum(sample_weights)
    df_sig_weight['weight'] = sample_weights
    # 计算处理后的权重直方图
    plt.hist(sample_weights, bins=100)
    plt.xlabel('Sample Weight')
    plt.ylabel('Frequency')
    plt.title('Histogram of Sample Weights After Adjustment')
    plt.savefig('sample_weights_after.png')
    plt.cla()
    ...
    sample_weights = bkg_pfff_total_weight['weight'].values
    # 计算处理前的权重直方图
    plt.hist(sample_weights, bins=100)
    plt.xlabel('Sample Weight')
    plt.ylabel('Frequency')
    plt.title('Histogram of Sample Weights Before Adjustment')
    plt.savefig('pfff_sample_weights_before.png')
    plt.cla()
    ...
    # 将所有样本的权重除以它们的总和，以使得它们的权重和为 1
    sample_weights = sample_weights / np.sum(sample_weights)
    bkg_pfff_total_weight['weight'] = sample_weights
    # 计算处理后的权重直方图
    plt.hist(sample_weights, bins=100)
    plt.xlabel('Sample Weight')
    plt.ylabel('Frequency')
    plt.title('Histogram of Sample Weights After Adjustment')
    plt.savefig('pfff_sample_weights_after.png')
    plt.cla()
