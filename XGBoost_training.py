...
def train_model(events_sig, events_bkg,colsample_bytree_v ,gamma_v, learning_rate_v, max_depth_v, min_child_weight_v, n_estimators_v, reg_alpha_v, reg_lambda_v, subsample_v,samples ):
    ...
    # mask_sig = np.logical_and(events_sig['CMS_hgg_mass'].array()>115,events_sig['CMS_hgg_mass'].array()<135)]
    # mask_dd = np.logical_and(events_bkg['DataDriven_QCD']['CMS_hgg_mass'].array()>115,events_bkg['DataDriven_QCD']['CMS_hgg_mass'].array()<135)
    # mask_pp = np.logical_and(events_bkg['pp']['CMS_hgg_mass'].array()>115,events_bkg['pp']['CMS_hgg_mass'].array()<135)]
    ...
    # mask = (df_sig['sigmarv']) < 0.008
    # sig_total_weight[mask] = sig_total_weight[mask] * 0.7142857
    # mask = (np.logical_and((df_sig['sigmarv']) < 0.008,(df_sig['sigmarv']) > 0.0076))
    # sig_total_weight[mask] = sig_total_weight[mask] * 10
    # sig_total_weight = df_sig_weight['weight']
    # sig_total_weight[sig_total_weight < 0.01] = 0
    ...
    # bkg_pp_total_weight[bkg_pp_total_weight>0.4] = 0
...
# events_sig_16preVFP_ggh = uproot.open("/hpcfs/cms/cmsgpu/zhangzhx/eos/UL2016PreVFP/output_ggh_125.root:tagsDumper/trees/ggh_125_13TeV_UntaggedTag")
# events_sig_16preVFP_ggh_new = uproot.open("/hpcfs/cms/cmsgpu/zhangzhx/eos/UL2016PreVFP/output_ggh_125.root:tagsDumper/trees/ggh_125_13TeV_UntaggedTag")
...
# train_model(events_sig=events_sig_16postVFP, events_bkg=events_bkg_16postVFP, colsample_bytree_v =  0.8834044009405149, gamma_v =  7.203244934421581, learning_rate_v =  0.0010113231069171437, max_depth_v =  4, min_child_weight_v =  1.4675589081711304, n_estimators_v =  7000, reg_alpha_v =  1.862602113776709, reg_lambda_v =  3.4556072704304777, subsample_v = 0.879353494846134, samples="UL16PostVFP")
